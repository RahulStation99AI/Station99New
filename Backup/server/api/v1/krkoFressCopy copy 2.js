...
var maxTry = 1;
function retry(res,dbName,advertFTPPath){
    if(maxTry<=5){
        maxTry++;  
        setTimeout(function () {
            advertData(res);
...
const advertData = async (res,dbName,advertFTPPath) => {
   ...
   // var advertFTPPath = '/Krko/Traffic/';
                    ...
                    if (typeof lastUpdatedData === "undefined") {
                        ...
                        for (const file of fileAdvertArray) {
                            ...
                            if (parseInt(lastSavedDate[0]) < parseInt(previousSavedDate[0])) {
                                  await returnFTPData(advertFTPPath,file,c,dbName);
                                 // console.log("async wait end");
                            }
                          }
                        
                        console.log('after forEach');
                        asyncCall('Cronjob Reminder for krko','Data for the krko has been imported');
    ...
    c.on('error', (err) => {
        asyncCall('Error on Krko Station99','FTP Client is not connect for Krko'+err);
        retry(res,dbName,advertFTPPath);
    });

  }
  
  async function returnFTPData(advertFTPPath,fileName,c,dbName){
        ...
        if(err !=  'Error: Unable to make data connection'){
           ...
           // console.log(insertAdvertValues);
            ...
            dbConnection.query(insertInfo, [insertAdvertValues], async function (err, result) {
                if (err) {
                    dbConnection.end();
                    asyncCall('Error on Krko Station99','advertisement Table is not connect '+err);
                } else {
                    ...
                    dbConnection.query(correctionQuery, async function (err, result) {
                        if (err) {
                            ...
                            asyncCall('Error on Krko Station99','last_updated_traffic Table is not connect '+err);
                        } else {
                            ...
                            dbConnection.query(lastUpdatedFileQuery, lastUpdatedFileValue, async function (err, result) {
                                if (err) {
                                    ...
                                    dbConnection.query(insertCronQuery, [cronValue], async function (err, result) {
                                        if (err) {
                                            dbConnection.end();
                                            asyncCall('Error on Krko Station99','cronlog Table is not connect '+err);
                                        } else {
...
router.get('/', function (req, res, next) {
    advertData(res,'station99_new','/Krko/Traffic/');
});
