...
router.get('/', async function (req, res, next) {
    ...
    maxResults: 1000,
        ...
        if(err){
          ...
          var checkRecordQuery = "select etag from connect_fm_playlist";
          dbConnection.query(checkRecordQuery, async function (error, resultData, fields) {
            if (error) {
              dbConnection.end();
            } else {
              for(var i = 0; i < resultData.length; i++){
                var etag = resultData[i].etag;
                getAllPlaylistReocrd(etag);
              }
 ...
 async function getAllPlaylistReocrd(etag){
