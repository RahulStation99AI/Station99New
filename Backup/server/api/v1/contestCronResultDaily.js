...
var setCron = [
    {
        stationName: "KRKO",
        dbName: 'station99_new'
    },{
        stationName: "KXA",
        dbName: 'kxa_new'
    },{
        stationName: "Connect FM",
        dbName: 'connect_fm'
    }
  ];
...
router.get('/',async function (req, res, next) {
    ...
    for (const eachFileItem of setCron) {
        console.log('eachFileItem.stationName : '+eachFileItem.stationName);
        mailSendStatus = false;
        await contestCronResultDaily(eachFileItem.stationName,eachFileItem.dbName);
    }
    res.send(JSON.stringify({ "status": 200, "error": null, "response":{},"message :": "data saved", "success": true }));
});

var contestCronResultDaily = async function (stationName,dbName)  {
            ...
            onestation.query(selectQuery,async function (err, result) {
                if (err) {
                    ...
                    for (const fileItem of result) {
                        ...
                        await getContestParticipant(currDate,contestId,contestName,contestDescription,companyEmail,dbName,stationName);
                    }
                    resolve(console.log("database "+dbName));
...
async function getContestParticipant(currDate,contestId,contestName,contestDescription,companyEmail,dbName,stationName){
