...
var maxTry = 1;
function retry(stationName,dbName,advertFTPPath){
    if(maxTry<=5){
        maxTry++;  
        setTimeout(function () {
            advertData(stationName,dbName,advertFTPPath);
        }, maxTry*60000);      
    }
    else
        maxTry=1;
...
const advertData = async (stationName,dbName,advertFTPPath) => {
   ...
   // var advertFTPPath = '/Krko/Traffic/';
    ...
    c.on('error', (err) => {
        ...
        retry(stationName,dbName,advertFTPPath);
    });
...
router.get('/', async function (req, res, next) {
    await advertData('KRKO','station99_new','/Krko/Traffic/');
    await advertData('KKXA','station99_test','/Kkxa/Traffic/');
 });
